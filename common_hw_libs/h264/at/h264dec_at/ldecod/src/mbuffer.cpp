...
#ifdef ESLCPP
  s->mb_field = (byte*)calloc (img->PicSizeInMbs, sizeof(int));
#else
...
//  printf ("output frame with frame_num #%d, poc %d (dpb. dpb.size=%d, dpb.used_size=%d)\n", dpb.fs[pos]->frame_num, dpb.fs[pos]->frame->poc, dpb.size, dpb.used_size);
  ...
  //printf("pos = %d dpb.fs[pos] = %d\n", pos, dpb.fs[pos]);//cylin.debug
 ...
 * \brief
 *    outputs one frame from dpb 
 ************************************************************************
 */
void flush_one_dpb()//added by cyline
{
  //printf("flush one frame from dpb\n");
  while (remove_unused_frame_from_dpb()) ;
  output_one_frame_from_dpb();
}
/*!
...
void alloc_ref_pic_list_reordering_buffer(Slice *currSlice)
{
  ...
  {
#ifdef ESLCPP
    if ((currSlice->remapping_of_pic_nums_idc_l0 = (int*)calloc(size,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: remapping_of_pic_nums_idc_l0");
    if ((currSlice->abs_diff_pic_num_minus1_l0 = (int*)calloc(size,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: abs_diff_pic_num_minus1_l0");
    if ((currSlice->long_term_pic_idx_l0 = (int*)calloc(size,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: long_term_pic_idx_l0");
#else
  ...
  {
#ifdef ESLCPP
    if ((currSlice->remapping_of_pic_nums_idc_l1 = (int*)calloc(size,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: remapping_of_pic_nums_idc_l1");
    if ((currSlice->abs_diff_pic_num_minus1_l1 = (int*)calloc(size,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: abs_diff_pic_num_minus1_l1");
    if ((currSlice->long_term_pic_idx_l1 = (int*)calloc(size,sizeof(int)))==NULL) no_mem_exit("alloc_ref_pic_list_reordering_buffer: long_term_pic_idx_l1");
#else
