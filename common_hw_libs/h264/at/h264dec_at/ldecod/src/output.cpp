...
unsigned char tlmDataBuffer[48];
unsigned int  tlmDataLenByte;
  ...
  ppm_width  = p->size_x - crop_right - crop_left;
      ...
      for(i=crop_top;i<p->size_y-crop_bottom;i++)
      {
        for(j=crop_left;j<p->size_x-crop_right;j++)
        {
          ...
          tlmDataBuffer[tlmDataLenByte+0] = CurColor[ 0];
          tlmDataBuffer[tlmDataLenByte+1] = CurColor[ 1];
          tlmDataBuffer[tlmDataLenByte+2] = CurColor[ 2];
          tlmDataLenByte += 3;

          if(tlmDataLenByte==48)
          {
